...
- Apply verifies written body fields after POST/PATCH, checking the write response first and then one fallback GET. Unreflected fields fail the item by default; use `--allow-unverified` with `--yes` only when an accepted-but-unproven write should remain best-effort.
